...
export default function ProfessorCourseViewPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (session, variables) => {
      ...
      setNewSessionTitle('')
      if (!variables.scheduled) navigate(`/sessions/${session.id}/report`)
    },
  })

  const endMutation = useMutation({
    mutationFn: (sessionId: string) => updateSessionStatus(sessionId, 'ended'),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['professor-sessions', courseId] }),
  })
  ...
  return (
    ...
    >
      ...
      >
        {successMessage && (
          <div className="mb-4 px-4 py-3 rounded-lg bg-green-500/15 text-green-700 border border-green-500/30 text-sm">
            {successMessage}
          </div>
        )}

        <h2 className="text-lg font-semibold text-foreground mb-1">Lectures</h2>
        <p className="text-sm text-muted-foreground mb-6">Manage your lectures</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
          ...
          >
            ...
            {activeSession ? (
              <div className="space-y-3">
                ...
                <Button variant="outline" size="sm" onClick={() => endMutation.mutate(activeSession.id)} disabled={endMutation.isPending} className="w-full">
                  End Lecture
                </Button>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                />
                <Button onClick={handleStartSession} disabled={createMutation.isPending} className="w-full gradient-primary text-white hover:opacity-90">
          ...
          <motion.button
            ...
            className="p-6 rounded-xl border-2 border-border bg-card text-left hover:border-primary/40 transition-all group"
          >
        ...
        {/* Lecture history */}
        <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Lecture History</h3>
              ...
              >
                ...
                {isUpcoming && (
                  <div className="absolute bottom-3 right-3 flex gap-1.5">
                    <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); navigate(`/instructor/courses/${courseId}/schedule/${session.id}`) }} className="text-xs h-7 px-2">
                      <Pencil className="h-3 w-3" />
                    </Button>
                    <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); handleDeleteLecture(session) }} disabled={deleteMutation.isPending} className="text-xs h-7 px-2 text-destructive hover:text-destructive">
                      <Trash2 className="h-3 w-3" />
                    </Button>
                    <Button size="sm" onClick={() => startMutation.mutate(session.id)} disabled={startMutation.isPending} className="gradient-primary text-white text-xs h-7 px-2">
